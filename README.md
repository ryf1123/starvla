...
从第一次跑通的 34% 到这里，中间有三次诊断——其中一次的因果判断后来被对照实验推翻了，
过程和更正都在 [notes/02](notes/02-闭环失败诊断.md)，那一页是本仓库最值得读的。
...
- **诊断出的异常 ≠ 病因**：我发现空间 softmax 接在 ReLU 之后时分布精确等于均匀分布（视觉分支"看起来是瞎的"），就把它当成了失败原因并一次改了三处。后来单独做对照才发现，那个"瞎了"的版本闭环成功率是 **87.5%**，比"修好"的版本（72.5%）还高。测量是真的，因果是我编的。踩坑全过程和原始错误推理都保留在 [notes/02](notes/02-闭环失败诊断.md)。

...
def make_env(kind, seed):
    if kind == "distract5":
        return TabletopEnv(seed=seed, n_cubes=5, n_plates=2)
    if kind == "plate3":
        return TabletopEnv(seed=seed, n_cubes=3, n_plates=3)
    return TabletopEnv(seed=seed, n_cubes=3, n_plates=2)

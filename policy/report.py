...
def load_run(d):
    ...
    if ev:
        ...
        row.update({k: c.get(k, 0) for k in ("wrong_cube", "no_grasp", "pushed", "dropped", "off_plate")})
...
def main():
    ...
    for r in rows:
        ...
        fail = (f"抓错{r.get('wrong_cube',0)} 没碰到{r.get('no_grasp',0)} "
                f"推走{r.get('pushed',0)} 掉了{r.get('dropped',0)} 没进盘{r.get('off_plate',0)}"
